...
A skill in `skills/` loads in full whenever it's triggered. Every line in it is a cost paid on
every one of those runs — so every line needs to earn that cost. This is the bar for writing one,
and for reviewing a `distill` proposal against.
...
`npm run check` runs `scripts/check-skills.mjs` — a frontmatter check, a line-count budget per
file (warning, not blocking), and a grep for the filler phrases above. It's advisory, the same way
the gates in `AGENTS.md` are for the agent: it tells you where you're off, it doesn't fail your
build for you.
