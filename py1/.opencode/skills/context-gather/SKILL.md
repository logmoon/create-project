...
At the start of a session, check the `context/` files for placeholder content (`<!-- ... -->` comments with nothing real around them, or the literal stub text shipped by `create-project`). If any required file is still a stub:
...
If all required files already have real content, skip this skill entirely and behave normally — do not re-run it on every session.
