...
This project's `.opencode/plugin/memory-hook.js` already injects a short summary of `memory.md` and `context/progress-tracker.md` at the start of every session, automatically. `/remember restore` is no longer required to avoid starting blank — it still has a job: it does the full read (including the per-agent context files below) and asks the developer to confirm the restored state before continuing. Run it when the auto-injected summary feels incomplete, when a session was interrupted abnormally, or any time you want an explicit checkpoint before proceeding.
